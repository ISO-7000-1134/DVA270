...
static Node * create_list_node(const Bucket data)
{
...
int number_of_list_nodes(const List list)
{
	// Empty list
	if (list == NULL)
		return 0;
...
static int search_from_head(const List head, const Bucket data) {
...
static int remove_element_from_head(List head, List* list, const Bucket data) {
	if(head->data.value == data.value) {
		// if (head->previous != NULL) {
		// 	head->previous->next = head->next;
